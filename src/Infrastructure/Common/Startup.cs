...
internal static class Startup
{
    internal static IServiceCollection AddServices(this IServiceCollection services, IConfiguration config)
    {
        ...
        if (settings.UsePersistence ?? false)
            services.AddServices(typeof(IPersistenceTransientService), ServiceLifetime.Transient);
        return services
            .AddServices(typeof(ITransientService), ServiceLifetime.Transient)
            .AddServices(typeof(IScopedService), ServiceLifetime.Scoped)
            .AddService(typeof(IJwtGenerator), typeof(JwtGenerator), ServiceLifetime.Scoped);
    }
